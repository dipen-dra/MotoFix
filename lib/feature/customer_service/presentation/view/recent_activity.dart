...
/// A widget that combines the main CTA card, quick services, and recent activity.
...
final List<Map<String, dynamic>> _recentActivities = [
  ...
  {'id': '3', 'title': 'Brake Pad Check', 'date': '2024-07-20', 'icon': FontAwesomeIcons.carBurst, 'status': 'Pending'},
  {'id': '4', 'title': 'Wash & Polish', 'date': '2024-07-18', 'icon': FontAwesomeIcons.soap, 'status': 'Cancelled'},
];
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        Column(
          children: _recentActivities.map((activity) => _buildActivityItem(context, activity)).toList(),
        ),
      ],
    );
  }
  ...
  Widget _buildActivityItem(BuildContext context, Map<String, dynamic> activity) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Icon(activity['icon'] as IconData?, color: AppColors.brandPrimary, size: 22),
            const SizedBox(width: 16),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(activity['title'] ?? '', style: const TextStyle(fontSize: 16, color: AppColors.textPrimary, fontWeight: FontWeight.w600)),
                  const SizedBox(height: 4),
                  Text(activity['date'] ?? '', style: const TextStyle(fontSize: 13, color: AppColors.textSecondary)),
                ],
              ),
            ),
            Text(
